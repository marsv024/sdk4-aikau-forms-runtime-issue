...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f artifact-share
	call %MVN_EXEC% clean package -pl artifact-share,artifact-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill artifact-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f artifact-acs
	call %MVN_EXEC% clean package -pl artifact-integration-tests,artifact-platform,artifact-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl artifact-platform,artifact-integration-tests,artifact-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl artifact-platform,artifact-integration-tests
EXIT /B 0
:purge
